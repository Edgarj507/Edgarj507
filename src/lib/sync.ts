...
import { PAR_BY_HOLE, COURSE } from '../data/course';
...
async function createRemoteRound(round: RoundState, visibility: Visibility, ownerId: string) {
 ...
 */
export function useRoundSync(round: RoundState, dispatch: Dispatch<RoundAction>, opts: { userId: string | null; visibility: Visibility }) {
  ...
  useEffect(() => {
    ...
    createRemoteRound(round, opts.visibility, opts.userId!)
